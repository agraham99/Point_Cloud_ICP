...
# Clouds are used as LAScatalog objects from the lidR package. Optimal matching is acheived when using clouds of similar point densities.  
# The program will spatially subsample both clouds to same point density with the SUBSAMPLE_DISTANCE parameter, the minimum distance between two points is generally best between 0.2-0.8 m.
...
# **GLOBAL_SHIFT_AUTO** - If True, CloudCompare will define a local coordinate system with global shifts. CloudCompare will preserve the absolute coordinates for any point cloud results exported.  
# If False, must supply global shift values below.
GLOBAL_SHIFT_AUTO = T
